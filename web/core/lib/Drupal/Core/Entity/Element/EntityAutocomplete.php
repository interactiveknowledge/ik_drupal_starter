    ...
    if (!empty($element['#value'])) {
      ...
      if (is_array($element['#value'])) {
        ...
        foreach ($input_values as $input) {
          ...
          // Exclude wrong match, when the entity label contains parentheses
          // (check if $match corresponds to the entity ID).
          if ($match !== NULL) {
            $valid_ids = $handler->validateReferenceableEntities([$match]);
            if (empty($valid_ids)) {
              $match = NULL;
            }
          }

...
(function ($, Drupal) {
  ...
  Drupal.ImageWidgetCropType.prototype.initializeCropper = function () {
    ...
    // Set saved crop area if 'Show default crop' is checked and a crop has been applied.
    if (this.options.autoCrop && this.values.applied.val() === '1') {
      var data = {};
      data.x = (this.naturalWidth * this.values.x.val()) / this.originalWidth;
      data.y = (this.naturalHeight * this.values.y.val()) / this.originalHeight;
      data.width = (this.naturalWidth * this.values.width.val()) / this.originalWidth;
      data.height = (this.naturalHeight * this.values.height.val()) / this.originalHeight;
